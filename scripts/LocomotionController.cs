...
public class LocomotionController : MonoBehaviour
{
    ...
    void Update()
    {
        if(leftTeleportRay)
        {
            ...
            teleportReticle.gameObject.SetActive(EnableLeftTeleport && CheckIfActivated(leftTeleportRay));
        }

        if (rightTeleportRay)
        {
            rightTeleportRay.gameObject.SetActive(EnableRightTeleport && CheckIfActivated(rightTeleportRay));
            teleportReticle.gameObject.SetActive(EnableRightTeleport && CheckIfActivated(rightTeleportRay));
        }
